...
export function useConnection() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error?.includes("not configured")) {
          setStatus("error")
          setError(
            "API key not configured. Please add GROQ_API_KEY to .env.local",
          )
        } else {
          setStatus("connected")
        }
  ...
  const reconnect = useCallback(async () => {
    if (reconnectAttempts >= CHAT_CONFIG.MAX_RECONNECT_ATTEMPTS) {
      setError(
        `Failed to reconnect after ${CHAT_CONFIG.MAX_RECONNECT_ATTEMPTS} attempts.`,
      )
      return
    }
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, CHAT_CONFIG.RECONNECT_DELAY),
    )
